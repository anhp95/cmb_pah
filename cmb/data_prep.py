...
def prep_ambient_data(ad_file_path="data/ambient/AD_merge_micg.xlsx"):
    species = [
        "Nap",
        "Acy",
        "Ace",
    ...
    used_species = [s for s in species if s not in NOT_USED_SPECIES]
    ...
    result_fine.to_csv("data/run/AD_fine.csv", index=False)
    result_coarse.to_csv("data/run/AD_coarse.csv", index=False)
    result.to_csv("data/run/AD.csv", index=False)

    return result, mean_df, std_df


def prep_source_profile(roi="delhi"):
    """
...
prep_ambient_data()
prep_source_profile()
prep_source_profile("seoul")
prep_source_profile_china()
# %%
